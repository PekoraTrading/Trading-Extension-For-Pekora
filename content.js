(async function () {
  ...
  function detectSharker() {
    const userIdElem = document.querySelector('a.aLink-0-2-129[href*="/User.aspx?ID="]');
    if (!userIdElem) return;
  
    const userId = userIdElem.href.split('?ID=')[1];
  
    fetch("https://raw.githubusercontent.com/PekoraTrading/Values-Demand/main/Sharks.json")
      .then(res => res.json())
      .then(data => {
        const isSharker = data.some(entry => entry.id === userId);
      ...
      .catch(err => {
        console.error("Error fetching shark list:", err);
      });
  }  
...
function updateRAPValues() {
  ...
  function addGoldEffect(element) {
    const style = document.createElement('style');
    style.innerHTML = `
      ...
      .gold-text {
        background: linear-gradient(90deg, #F1C40F, #FFCE3D);
        background-size: 300% 300%;
        color: transparent;
        -webkit-background-clip: text;
        background-clip: text;
  ...
  function addGreenEffect(element) {
    const style = document.createElement('style');
    style.innerHTML = `
      ...
      .green-text {
        background: linear-gradient(90deg, #04B959, #037A3B);
        background-size: 300% 300%;
        color: transparent;
        -webkit-background-clip: text;
        background-clip: text;
        animation: green-gradient 4s ease infinite;
      }
    `;
    document.head.appendChild(style);
  ...
  function addRedEffect(element) {
    ...
    style.innerHTML = `
      ...
      .red-text {
        background: linear-gradient(90deg, #F62727, #AD1B1B);
        background-size: 300% 300%;
        color: transparent;
        -webkit-background-clip: text;
        background-clip: text;
        animation: red-gradient 4s ease infinite;
      }
    `;
    document.head.appendChild(style);
  ...
  function addPurpleEffect(element) {
    const style = document.createElement('style');
    style.innerHTML = `
      ...
      .purple-text {
        background: linear-gradient(90deg, #F62727, #65006B);
        background-size: 300% 300%;
        color: transparent;
        -webkit-background-clip: text;
        background-clip: text;
        animation: purple-gradient 4s ease infinite;
      }
    `;
    document.head.appendChild(style);
  ...
  function addOrangeEffect(element) {
    const style = document.createElement('style');
    style.innerHTML = `
      ...
      .orange-text {
        ...
        -webkit-background-clip: text;
        background-clip: text;
        animation: orange-gradient 4s ease infinite;
      }
    `;
    document.head.appendChild(style);
  ...
  function addTealEffect(element) {
    const style = document.createElement('style');
    style.innerHTML = `
      ...
      .teal-text {
        ...
        -webkit-background-clip: text;
        background-clip: text;
        animation: teal-gradient 4s ease infinite;
      }
    `;
    document.head.appendChild(style);
  ...
  function checkAndEnhanceProfilePage() {
    ...
    const url = `https://raw.githubusercontent.com/PekoraTrading/Values-Demand/main/users.json?timestamp=${timestamp}`;
    ...
    fetch(url)
      ...
      .then(data => {
        ...
        if (usernameElement && userEntry) {
          ...
          } else if (userEntry.effect === 'gold') {
            addGoldEffect(usernameElement);
          } else if (userEntry.effect === 'green') {
            addGreenEffect(usernameElement);
          } else if (userEntry.effect === 'red') {
            addRedEffect(usernameElement);
          } else if (userEntry.effect === 'purple') {
            addPurpleEffect(usernameElement);
          } else if (userEntry.effect === 'orange') {
            addOrangeEffect(usernameElement);
          } else if (userEntry.effect === 'teal') {
            addTealEffect(usernameElement);
          }
        }
      });
  }  
